...
const CsvApiService = require("./CsvApiService");
const csvApiService = new CsvApiService();
const path = require("path");
const S3CSVService = require("../utils/S3CSVService");
const STOREFILEWRITEPATH = path.join(
  __dirname,
  "../file_structure/product_dump/product.json"
);
...
class ProductService {
  ...
  async getProductsFromCSV(query) {
    try {
  ...
  async getProductBySearch(search, id, name, type) {
    ...
    if (id && id !== "0" && name !== "-") {
      ...
      cid =
        "172122.156074.815D5727FF79F9D1,172122.156074.477079660CE9C556,158527.1.4D5,209002.156074.1310692960C87047,200434.156074.7A6D0F7C12820A28,181293.156074.E474BA81234AD025,204122.156052.207C,192065.156052.219F";
    }
    ...
    try {
      // Fetch product IDs
      const productIds = await product.getFlexOfferProductIds(
        FLEX_OFFER_API1,
        flexOfferHeader
      );
      const responseData = [];
      ...
      for (const pid of productIds) {
        ...
        try {
          const fullProductDetailsArray = await product.getAPI(
            productDetailsAPI,
            flexOfferHeader,
            "JSON"
          );
          ...
          if (
            ...
            if (
              ...
              ((analyzedQuery.item &&
                new RegExp(`\\b${analyzedQuery.item}\\b`, "i").test(
      ...
      if (analyzedQuery.gender === "GIRL" || analyzedQuery.gender === "GIRLS") {
        ...
        analyzedQuery.gender === "BOY" ||
        analyzedQuery.gender === "BOYS"
      ) {
      ...
      apiDataLinkShare.result.item.forEach((prod) => {
        ...
        if (
          !uniqueMap.has(prod.imageurl[0]) &&
          currency === "USD" &&
          (targetWordRegix.test(prod.prductname[0]) ||
            targetWordRegix.test(prod.description[0].short[0]))
        ) {
          ...
          const responseStructure = {
            ...
            discount: "0%",
            category:
              prod.category[0].primary[0] +
              "->" +
              prod.category[0].secondary[0],
            manufacturer: "-",
  ...
  async fetchCIDsAndUpdateJSON() {
    ...
    const flexofferStores = jsonData.filter(
      (store) => store.from === "FLEXOFFER" && !store.cid
    );
    ...
    if (flexofferStores.length > 0) {
      ...
      for (const store of flexofferStores) {
        ...
        try {
          ...
          console.error(
            `Error fetching CIDs for store ID ${store.store_id}:`,
            error.message
          );
        }
      }
  ...
  createProductResponseStructure(productDetails, source) {
    let advertiserName =
      productDetails.advertiserName ||
      productDetails.merchantname?.[0] ||
      "N/A";
    if (
      (productDetails.advertiserName ||
        productDetails.merchantname?.[0] ||
        "N/A") === "Bergdorf Goodman (Neiman Marcus)"
    ) {
    ...
    let price = parseFloat(
      productDetails.price?.[0]?._ || productDetails.price || 0
    );
    let salesPrice = parseFloat(
      productDetails.salePrice || productDetails.saleprice?.[0]?._ || 0
    );
    ...
    return {
      productName:
        productDetails.name || productDetails.productname?.[0] || "N/A",
      ...
      currency:
        ...
        productDetails.price?.[0]?.$?.currency ||
        "USD",
      ...
      urlName: urlName,
      description:
        productDetails.description?.[0]?.short?.[0] ||
      ...
      isInStock:
        productDetails.isInstock !== undefined
  ...
  async searchProductsByKeywords(shop, keywords, page = 1, pageSize = 10) {
    ...
    const mappingsFilePath = path.join(
      __dirname,
      "../file_structure/keyword_dump/keywordMappings.json"
    );
    const keywordMappings = JSON.parse(
      fs.readFileSync(mappingsFilePath, "utf-8")
    );
